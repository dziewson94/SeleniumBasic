...
public class ResizableTest extends TestBase {
    ...
    public void resizableTest(String url, List<Map<Direction, Integer>> steps) {
        pageHelper.init(url);
        ...
        Dimension resizableOrginalSize = resizable.getSize();
        int stepIndex = 1;
        for (Map<Direction, Integer> step : steps) {
            ...
            if (step.size() == 1) {
                resizableOrginalSize = resizable.getSize();
                ...
                if (direction == Direction.RIGHT) {
                 ...
                 //   assertThat(resizable.getSize().getWidth()).isGreaterThan(resizableOrginalSize.getWidth());
                   // assertThat(resizable.getSize().getWidth() - resizableOrginalSize.getWidth()).isEqualTo(offset);
                }else if(direction == Direction.DOWN) {
                    logger.info(String.format("Element size after step %d is %s",stepIndex, resizable.getSize().toString()));
                    logger.info(String.format("Element location after step %d is %s",stepIndex, resizable.getLocation().toString()));
//                    assertThat(resizable.getSize().getHeight()).isGreaterThan(resizableOrginalSize.getHeight());
  //                  assertThat(resizable.getSize().getHeight() - resizableOrginalSize.getHeight()).isEqualTo(offset);
                }
            } else {
                resizableOrginalSize = resizable.getSize();
...
//                assertThat(resizable.getSize().getWidth()).isGreaterThan(resizableOrginalSize.getWidth());
//                assertThat(resizable.getSize().getWidth() - resizableOrginalSize.getWidth()).isEqualTo(secondRecord.getValue());
//                assertThat(resizable.getSize().getHeight()).isGreaterThan(resizableOrginalSize.getHeight());
//                assertThat(resizable.getSize().getHeight() - resizableOrginalSize.getHeight()).isEqualTo(firstRecord.getValue());
            }
        }
        logger.info("");
    }
